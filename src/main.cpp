...
#if defined(NDEBUG)
# error "DAN$coin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x95fa188e2ec72eaeb7bdaa6ae2a51f5b5f560e9dc305d212d60b9c8196250667");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DAN$coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DAN$coin Signed Message:\n";
...
{
    // DAN$coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // DAN$coin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // DAN$coin retarget time: 3.5 days (1 day x 24 hours x 60 minutes x 60 seconds)
static const int64 nTargetSpacing = 5 * 60; // DAN$coin block production time: 5 minutes (5 minutes x 60 seconds)
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DAN$coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DAN$coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8ab93524cb198cafda59091f7547087f5098141f509cc8f93bcc882256e6d11d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BRASIL, 07/04/2018, Lula foi preso, Povo liberto";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 4 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045306f526fbbd2e0a56c2fd242cd54384183d3a96a884b5199f7930354465f17f78eaf19b367a7aab734bd89858d6b1738f08ad72b0e898b938f80f7495122b8d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523759844;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087097286;

        if (fTestNet)
        {
            block.nTime    = 1523759779;
            block.nNonce   = 387404384;
        }
	
	// start ---- CODE TO GENERATE THE GENSIS BLOCK (true/false) ---- start
	...
	// end ---- CODE FOR MINE THE GENSIS BLOCK ---- end
        ...
        assert(block.hashMerkleRoot == uint256("0x2b1948e6e024b76509682a668626e7ba5b797a2d6a26776b4cfe741917e81129"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xb9, 0xd7 }; // DAN$coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DAN$coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DAN$coinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DAN$coinMiner(CWallet *pwallet)
{
    printf("DAN$coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("danscoin-miner");
    ...
    try { loop {
        ...
        printf("Running DAN$coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DAN$coinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DAN$coinMiner, pwallet));
}
